...
//             Write each word (multi-byte entry) word-by-word
            for (addr = 0; addr < mem_depth; addr = addr + 1) begin
                mem_addr = addr;               
                if (block >= 6) begin
                    for (byte = 0; byte < X_IN; byte = byte + 1) begin
                        wrd_addr = byte;  // word-level offset
                        byte_val = (addr + byte) % 10;  // example pattern
                        data_in = byte_val[DATA_WIDTH - 1 : 0];
                        #10;
                    end            
                end else begin 
                    if(block < 3) begin
                        for (byte = 0; byte < NUM_QKV_MACS * NUM_QKV_PES * NUM_QKV_ROWS; byte = byte + 1) begin
                            wrd_addr = byte;  // word-level offset
                           byte_val = (addr + byte) % 256;  // example pattern
                            data_in = byte_val[DATA_WIDTH - 1 : 0];
                            #10;
                        end
                    end else begin
                        for (byte = 0;  byte < NUM_FF_MACS * NUM_FF_PES * NUM_FF_ROWS; byte = byte + 1) begin
                            wrd_addr = byte;  // word-level offset
                           byte_val = (addr + byte) % 256;  // example pattern
                            data_in = byte_val[DATA_WIDTH - 1 : 0];
                            #10;
